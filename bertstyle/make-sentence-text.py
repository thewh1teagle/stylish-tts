...
def seek_sentence(index, phrases, chapter_length):
    ...
    if index < len(phrases):
        ...
        while not done:
            can_lookahead = (index < len(phrases) - 1
                             and phrases[index+1][2] is not None)
    ...
    with filename.open(mode="r") as f:
        ...
        for line in f:
            ...
            if fields[0] == "chapter":
                ...
                chapters[name].append((int(fields[1].strip()),
                                       int(fields[2].strip()),
                                       fields[3].strip()))
            else:
    ...
    with torch.no_grad():
        ...
        for key in chapters.keys():
            ...
            while index < len(phrases):
                (index, begin, end, text) = seek_sentence(index, phrases, chapter_length)
                if (len(text) > 0
                    and len(text) < 500
        ...
        numpy.savez_compressed("sentence-data.npz",
                               allow_pickle=False,
                               **data)
        valfile.close()
        trainfile.close()
