...
def ctc_loss(
    ...
    diff_labels = torch.cat(
        [
            torch.tensor([[False, False]], device=targets.device).expand(
                batch_size, -1
            ),
    ...
    log_alpha = torch.full(
        (input_time_size, batch_size, zero_padding + _t_a_r_g_e_t_s_.shape[-1]),
        zero,
        device=log_probs.device,
        dtype=log_probs.dtype,
    )
    ...
    for t in range(1, input_time_size):
        log_alpha[t, :, 2:] = log_probs_[t] + logadd(
            log_alpha[t - 1, :, 2:],
            log_alpha[t - 1, :, 1:-1],
            torch.where(diff_labels, log_alpha[t - 1, :, :-2], zero),
        )

    l1l2 = log_alpha[input_lengths - 1, B].gather(
        -1,
        torch.stack(
            [zero_padding + target_lengths * 2 - 1, zero_padding + target_lengths * 2],
            dim=-1,
        ),
    )
    ...
    path = torch.zeros(
        len(log_alpha), len(B), device=log_alpha.device, dtype=torch.int64
    )
    path[input_lengths - 1, B] = (
        zero_padding + 2 * target_lengths - 1 + l1l2.argmax(dim=-1)
    )
    for t, indices in reversed(list(enumerate(path))[1:]):
        indices_ = torch.stack(
            [
                (indices - 2) * diff_labels[B, (indices - zero_padding).clamp(min=0)],
                (indices - 1).clamp(min=0),
                indices,
            ],
            dim=-1,
        )
        path[t - 1] += (
            indices - 2 + log_alpha[t - 1, B].gather(-1, indices_).argmax(dim=-1)
        ).clamp(min=0)
    return torch.zeros_like(log_alpha).scatter_(-1, path.unsqueeze(-1), 1.0)[
        ..., (zero_padding + 1) :: 2
    ]
...
def ctc_alignment(
    ...
    log_alpha = torch.full(
        (batch_size, padded_t), zero, device=log_probs.device, dtype=log_probs.dtype
    )
    ...
    backpointers_shape = [
        len(log_probs),
        batch_size,
        int(math.ceil(padded_t / packnibbles)),
    ]
    backpointers = torch.zeros(
        backpointers_shape, device=log_probs.device, dtype=torch.uint8
    )
    backpointer = torch.zeros(
        (backpointers_shape[-2], backpointers_shape[-1] * packnibbles),
        device=log_probs.device,
        dtype=torch.uint8,
    )
    packshift = torch.tensor(
        [[[6, 4, 2, 0]]], device=log_probs.device, dtype=torch.uint8
    )
    ...
    for t in range(1, input_time_size):
        prev = torch.stack(
            [
                log_alpha[:, 2:],
                log_alpha[:, 1:-1],
                torch.where(diff_labels, log_alpha[:, :-2], zero),
            ]
        )
        log_alpha[:, zero_padding:] = log_probs[t].gather(
            -1, _t_a_r_g_e_t_s_
        ) + prev.logsumexp(dim=0)
        backpointer[:, zero_padding : (zero_padding + prev.shape[-1])] = prev.argmax(
            dim=0
        )
        torch.sum(
            backpointer.unflatten(-1, (-1, packnibbles)) << packshift,
            dim=-1,
            out=backpointers[t],
    ...
    path = torch.zeros(
        input_time_size, batch_size, device=log_alpha.device, dtype=torch.int64
    )
    path[input_lengths - 1, B] = (
        zero_padding + target_lengths * 2 - 1 + l1l2.argmax(dim=-1)
    )

    for t in range(input_time_size - 1, 0, -1):
        indices = path[t]
        backpointer = (backpointers[t].unsqueeze(-1) >> packshift).view_as(
            backpointer
        )  # backpointer = backpointers[t]
        path[t - 1] += indices - backpointer.gather(-1, indices.unsqueeze(-1)).squeeze(
            -1
        ).bitwise_and_(packmask)

    return torch.zeros_like(_t_a_r_g_e_t_s_, dtype=torch.int64).scatter_(
        -1,
        (path.t() - zero_padding).clamp(min=0),
        torch.arange(input_time_size, device=log_alpha.device).expand(batch_size, -1),
    )[:, 1::2]
...
def ctc_alignment_uncompressed(
    ...
    zero, zero_padding = (
        torch.tensor(
            finfo_min_fp16 if log_probs.dtype is torch.float16 else finfo_min_fp32,
            device=log_probs.device,
            dtype=log_probs.dtype,
        ),
        2,
    ...
    padded_t = zero_padding + _t_a_r_g_e_t_s_.shape[-1]
    log_alpha = torch.full(
        (len(B), padded_t), zero, device=log_probs.device, dtype=log_probs.dtype
    )
    log_alpha[:, zero_padding + 0] = log_probs[0, :, blank]
    log_alpha[:, zero_padding + 1] = log_probs[0, B, _t_a_r_g_e_t_s_[:, 1]]
    ...
    backpointers = torch.zeros(
        (
            backpointers_shape
            if not pack_backpointers
            else (backpointers_shape[:-1] + (padded_t // packnibbles,))
        ),
        device=log_probs.device,
        dtype=torch.uint8,
    )
    backpointer = torch.zeros(
        backpointers_shape[1:], device=log_probs.device, dtype=torch.uint8
    )
    packshift = torch.tensor(
        [[[6, 4, 2, 0]]], device=log_probs.device, dtype=torch.uint8
    )
    ...
    for t in range(1, len(log_probs)):
        ...
        log_alpha[:, 2:] = log_probs[t].gather(-1, _t_a_r_g_e_t_s_) + prev.logsumexp(
            dim=0
        )
        backpointer[:, 2 : (2 + prev.shape[-1])] = prev.argmax(dim=0)
        if pack_backpointers:
            torch.sum(
                backpointer.view(len(backpointer), -1, 4) << packshift,
                dim=-1,
                out=backpointers[t],
            )
        else:
    ...
    path = torch.zeros(
        len(log_probs), len(B), device=log_alpha.device, dtype=torch.int64
    )
    path[input_lengths - 1, B] = (
        zero_padding + target_lengths * 2 - 1 + l1l2.argmax(dim=-1)
    )
...
def ctc_alignment_targets(
    ...
    loss = ctc_loss(
        log_probs, targets, input_lengths, target_lengths, blank=blank, reduction="sum"
    )
    probs = log_probs.exp()
    ...
    temporal_mask = (
        torch.arange(
            len(log_probs), device=input_lengths.device, dtype=input_lengths.dtype
        ).unsqueeze(1)
        < input_lengths.unsqueeze(0)
    ).unsqueeze(-1)
