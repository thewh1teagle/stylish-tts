...
from .distributed import all_gather_variable_dim, AllGather

from .ring import get_rank, get_world_size
...
def zig_zag_pad_seq(t):
    ...
    t = F.pad(t, (0, 0, 0, padded_seq_len - seq_len), value=0.0)
...
ShardOutput = namedtuple(
    "ShardOutput", ["local_sequence", "query_positions", "key_value_positions"]
)
...
def zig_zag_shard(t, all_gather_batch=False):
    ...
    def inverse(two_chunks):
        ...
        first_half, second_half = rearrange(
            all_chunks, "b (all_pairs two) c d -> two b all_pairs c d", two=2
        )
...
def zig_zag_attn(
    ...
    out = F.scaled_dot_product_attention(
        q, k, v, dropout_p=dropout, attn_mask=attn_mask
    )
