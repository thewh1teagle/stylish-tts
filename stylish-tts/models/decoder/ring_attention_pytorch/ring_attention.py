...
from .ring import all_ring_pass, is_distributed, get_rank, get_world_size

from .ring_flash_attention import ring_flash_attn

from .distributed import split_by_rank, AllGather
...
class RingRotaryEmbedding(Module):
    ...
    @beartype
    def forward(self, seq: int | Tensor):
        device = self.device
...
def pad_at_dim(t: Tensor, pad: Tuple[int, int], *, dim=-1, value=0.0):
...
def maybe_pad_seq_and_mask(x: Tensor, mask: Tensor | None, seq_size: int):
...
def sharded_batch_to_sharded_seq(x: Tensor, mask: Tensor | None, seq_size: int):
...
class RingAttention(Module):
    ...
    def __init__(
        ...
        *,
        ...
        assert divisible_by(
            heads, num_grouped_query_heads
        ), f"number of query heads ({heads}) must be divisible by the groups ({num_grouped_query_heads})"
        ...
        assert (not ring_attn) or divisible_by(
            ring_seq_size, bucket_size
        ), f"ring seq size {ring_seq_size} is not divisible by bucket size {bucket_size}"
    ...
    def forward(
        ...
        striped_bucket_size = (
            self.bucket_size if not using_striped_ring_cuda else self.ring_seq_size
        )
        ...
        q, k, v = rearrange(qkv, "b n (h d) -> b n h d", d=self.dim_head).split(
            self.qkv_head_breakdown, dim=-2
        )
        ...
        if self.force_regular_attn:
            ...
            from .ring_flash_attention_cuda import ring_flash_attn_cuda
    ...
    def forward(
        ...
        if auto_shard_seq:
            ...
            if self.striped_ring_attn:
                ...
                if exists(labels):
                    labels = rearrange(
                        labels, "b (i j) -> b (j i)", i=striped_bucket_size
                    )
