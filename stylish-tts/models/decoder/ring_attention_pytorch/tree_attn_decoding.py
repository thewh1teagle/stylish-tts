...
def tree_attn_decode(
    ...
    assert not (
        exists(k) ^ exists(v)
    ), "keys and values are either both None, or both present"
    ...
    if shard_kv_seq:
        assert exists(
            k
        ), "keys and values must be passed if not already sharded across sequence"
        dim_v = v.shape[-1]
    ...
    if exists(k):
        ...
        assert not (
            use_triton and not q.is_cuda
        ), "input needs to be on cuda if forcing the use of triton"
