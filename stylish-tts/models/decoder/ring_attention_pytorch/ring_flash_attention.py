...
class RingFlashAttentionFunction(Function):
    ...
    def forward(
        ...
        for (ring_rank, _), ((kv, mask), (receive_kv, receive_mask)) in ring_pass_fn(
            kv,
            mask,
                ...
                for ind, (qc, oc, row_sums, row_maxes) in enumerate(row_splits):
                    ...
                    if exists(col_mask):
                        ...
                        attn_weights = attn_weights.masked_fill(
                            ~col_mask_unsqueezed, max_neg_value
                        )
                    ...
                    if exists(col_mask):
                        exp_weights = exp_weights.masked_fill(~col_mask_unsqueezed, 0.0)
        ...
        for (ring_rank, _), (
            ...
            for k_ind, (kc, vc, dkc, dvc, col_mask) in enumerate(col_splits):
                ...
                for ind, (qc, doc, Dc, lsec, dqc) in enumerate(row_splits):
                    ...
                    if exists(col_mask):
                        col_mask_unsqueezed = rearrange(col_mask, "b j -> b 1 1 j")
                        p = p.masked_fill(~col_mask_unsqueezed, 0.0)
